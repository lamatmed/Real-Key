...
function PropertyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const property = useQuery(api.properties.getProperty, { id: unwrappedParams.id as any })

  const deleteProperty = useMutation(api.properties.deleteProperty)

  const handleDelete = async () => {
    try {
      await deleteProperty({ id: unwrappedParams.id as any })
        ...
        {property?.images && property.images.length > 0 ? (
          <div className='space-y-4'>
            ...
            <div className='relative h-96 w-full mt-10'>
              <Image alt='img' width={100} height={1500} className="object-cover w-full rounded-lg h-[550px]" src={property?.images[selectedImageIndex]} />
            </div>
